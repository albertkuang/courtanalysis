...
async function main() {
    ...
    finalPlayers.slice(0, 20).forEach((p, i) => {
        console.log(`${String(i + 1).padStart(3)}. ${p.name.substring(0, 25).padEnd(25)} UTR: ${String(p.utr).padEnd(6)} ${(p.nationality || '').padEnd(4)} ${p.proRank.padEnd(12)}`);
    });
    ...
    const headers = ['Rank', 'Name', 'Singles UTR', 'Doubles UTR', '3-Month Trend', 'Age', 'Country', 'Location', 'Pro Rank', 'College', 'Profile URL'];
